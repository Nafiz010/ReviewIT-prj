...
<body style="display: flex;flex-direction: column; position: relative; padding-bottom: 120px;">
    ...
    <div class="container">
        <h1 class="text-center"><hr>View Available Restaurants <hr></h1>
    </div>
        ...
        if ($check_outlet) {
            while($row = mysqli_fetch_array($query_run))
            {
                ...
                <div class="container mt-5">
                    <div class="row">
                        <div class="col-md-4">
                            <img src="../img/<?= $row['logo'];?>" style="width:255px; height:250px" alt="Outlet Image">
                        </div>
                        <div class="col-md-7 ml-5">
                            <h2 class="fw-bold">Outlet Name : <?= $row['name']; ?></h2>
                            <hr>
                            <h4>Description : <?= $row['description'];?></h4>
                            <hr>
                            <hr><h4>Phone Number : <?= $row['phone'];?></h4>
                            <hr>
                            <hr><h4>Location : <?= $row['address'];?></h4>
                            <hr>
                            <button class="btn btn-dark"><a href="home-outlet-info.php?id=<?= $row['id']; ?>" style="color: white; text-decoration:none"><i class="fa fa-shopping-cart me-2"></i>View Outlet</a></button>
                        </div>
                    </div><br><hr>
                </div>
